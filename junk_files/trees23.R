...
par2<-subset(par2,nchar(par2$SC)>=20)
allans<-pblapply(par2$S,par2ans1)
...
y1<-lapply(corpus1[sapply(corpus1,function(x) length(content(x)))>0],function(k){list(k,annotate(as.String(content(k)),sent_token_annotator))})
...
nrow(par1)
par1$S[-processed$docs.removed][out$docs.removed][10000]
which(as.character(par1$S)==as.character(corpEntS[2]))


tcs<-lapply(tps, function(k){
