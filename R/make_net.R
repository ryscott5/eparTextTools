...
data.frame("clust"=1:500,"newword"=unlist(sapply(1:500,function(k) names(kverbs$cluster[kverbs$cluster==k])[which.max(kverbs$centers[k,c(which(kverbs$cluster==k))])])))


kverbs$cluster[kverbs$cluster==1]
